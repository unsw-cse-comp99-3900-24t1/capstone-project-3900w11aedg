      ...
      <ScrollView className="mb-20">
        ...
        <View className="h-[65%] flex items-center space-y-5">
          <LinearGradient colors={gradientColour} className="rounded-2xl">
            <View className="h-fit w-80 rounded-md overflow-hidden p-4">
              <View className="px-4 py-2">
                <Text className="text-2xl text-white font-bold">{card.name}</Text>
                {card.description.length > 0 && (
                  <Text className="text-base text-white my-6">{card.description}</Text>
                )}
                <Text className="text-white font-bold text-xl">{card.type}</Text>
                <View className="flex-row justify-between items-center">
                  <Text className="text-white text-lg mr-4">
          ...
          <View className="w-[90%] bg-card-view-grey rounded-2xl space-y-5 p-5">
            ...
            <View className="w-80 bg-popup-grey flex-row p-3 space-x-5 rounded-2xl">
              <View className="">
                ...
                {Object.keys(card.claims).map((claim: string, index: number) => (
                  <Text key={index} numberOfLines={1} className="text-white font-medium">
                    {normaliseKey(claim)}
                  </Text>
                ))}
              </View>
              <View className="flex-1">
                <Text numberOfLines={1} className="text-grey">
                ...
                {Object.values(card.claims).map((claim: string, index: number) => (
                  <Text key={index} numberOfLines={1} className="text-grey">
                    {claim}
                  </Text>
                ))}
              </View>
