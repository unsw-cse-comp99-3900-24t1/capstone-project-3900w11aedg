...
const mockCredential: VerifiableCredential = {
  ...
  'proof': {
    ...
    'proofValue': 'u2V0ChVhQlfPqJYDn9CGSsOjoFuWjDhDKJuiIYxL-8N8Y_Vtr8qVmwwwb0qFXYzxYOPVfh-6ZQy1qHhPAasetBjb3w0PoZ5IN_eaoosYGwKntG0QFyV9YQKsM9n7NaM9xqghRfvkEUXfa_4HrZy5hV6EbGDyu_RdmxrB_jxPMJBo8O5QaVN_ZgO9NVqPLpiOrrpoHsjHipblYYI69z7jeXZb1bsoHv2a60GX-WSHCQ5q67_cyNjo5Ru-sKlHitsZTa7iM27HjwxE0CA8eFWSEYC4b8lykA81ettkT2pamEDbrGdGTrIUn1eYizWNGU1GbOHXrBaGBCxhzzVggQHd1gwBtO5WaO4B7uKWVqPz3YhuBUqLFxE7sXgpFJNyCbS9pc3N1YW5jZURhdGVnL2lzc3Vlcg',
  },
};
...
describe('POST /presentation/create', () => {
  ...
  it('Credentials or serviceProviderUrl is missing', async () => {
    ...
    res = await request(app.default).post('/presentation/create').send({ credentials: [mockCredential] });
    expect(res.status).toBe(400);
    expect(res.text).toBe('Missing credentials or service provider URL');
    res = await request(app.default).post('/presentation/create').send({ serviceProviderUrl: 'http://example.com' });
    expect(res.status).toBe(400);
    expect(res.text).toBe('Missing credentials or service provider URL');
  });

  it('Credentials or serviceProviderUrl are in the wrong format', async () => {
    const res = await request(app.default).post('/presentation/create').send({
  ...
  it('ClaimsToKeep exists but is empty or not an array', async () => {
    let res = await request(app.default).post('/presentation/create').send({
    ...
    res = await request(app.default).post('/presentation/create').send({
      credentials: [mockCredential],
  ...
  it('Creates a presentation and is sent and verified by the service provider', async () => {
    ...
    (deriveAndCreatePresentation as jest.MockedFunction<typeof deriveAndCreatePresentation>).mockResolvedValueOnce(
      mockPresentation,
    );
    const axios = (await import('axios')).default;
    const { post } = axios;
    (post as jest.MockedFunction<typeof post>).mockResolvedValueOnce({ status: 200, data: { valid: true } });
    const res = await request(app.default).post('/presentation/create').send({
      credentials: [mockCredential],
      serviceProviderUrl: 'http://example.com',
    });
  ...
  it('Creates a presentation and is sent to service provider but is invalid', async () => {
    ...
    const axios = (await import('axios')).default;
    const { post } = axios;
    (post as jest.MockedFunction<typeof post>).mockResolvedValueOnce({ status: 400, data: { valid: false } });
    const res = await request(app.default).post('/presentation/create').send({
      credentials: [{
        ...mockCredential,
        proof: {
