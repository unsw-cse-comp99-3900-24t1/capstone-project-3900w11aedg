...
export const saveData = (didURL = path.join(__dirname, 'did.txt'), keyPairURL = path.join(__dirname, 'keyPair.key'), 
// eslint-disable-next-line @typescript-eslint/no-explicit-any
keyPair, did) => {
    fs.writeFileSync(didURL, did);
    fs.writeFileSync(keyPairURL, JSON.stringify(keyPair, null, 2));
};
export const loadData = async (didURL = path.join(__dirname, 'did.txt'), keyPairURL = path.join(__dirname, 'keyPair.key')) => {
